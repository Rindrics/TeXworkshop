...
[fn: 本当は，これはデータベースではなく計算結果なのですがここではデータベースということにします]
で，
列は左から，年，資源量，産卵親魚量，再生産成功率，そして漁獲率です．
#+BEGIN_SRC R :results silent :session *TeXws* :exports code
stockdata <- read.csv("ex2/data/Timeseries_TW_Engraulis-japonicus.csv")
head(stockdata)
#+END_SRC
#+BEGIN_SRC R :results table :exports result
stockdata <- read.csv("ex2/data/Timeseries_TW_Engraulis-japonicus.csv", header = FALSE)
head(stockdata)
#+END_SRC
...
| 1980 |        95 |          64 |             855 |              51 |
| 1981 |       101 |          64 |             821 |              46 |
R の [[https://cran.r-project.org/web/packages/Hmisc/][Hmisc パッケージ]] を使って，csvデータを加工していきます
*** TeX で.tex を読み込む
** 文献リストの準備
.bibtexファイル
** マクロをstyとして書き出す
